...
class ShopSettings(Document):
	...
	def validate(self):
		...
		frappe.clear_document_cache("Shop Settings", "Shop Settings")

		self.is_redisearch_enabled_pre_save = frappe.db.get_single_value(
			"Shop Settings", "is_redisearch_enabled"
		)
...
def validate_cart_settings(doc=None, method=None):
	frappe.get_doc("Shop Settings", "Shop Settings").run_method("validate")


def get_shopping_cart_settings():
	return frappe.get_cached_doc("Shop Settings", ignore_permissions=True)
