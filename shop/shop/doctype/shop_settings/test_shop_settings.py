...
class TestShopSettings(unittest.TestCase):
	...
	def test_tax_rule_validation(self):
		...
		cart_settings = frappe.get_doc("Shop Settings")
	...
	def test_invalid_filter_fields(self):
		"Check if Item fields are blocked in Shop Settings filter fields."
		from frappe.custom.doctype.custom_field.custom_field import create_custom_field

		setup_shop_settings({"enable_field_filters": 1})
...
def setup_shop_settings(values_dict):
	"Accepts a dict of values that updates Shop Settings."
	if not values_dict:
		return

	doc = frappe.get_doc("Shop Settings", "Shop Settings")
