frappe.provide("shop.shop.wishlist");
var wishlist = shop.shop.wishlist;
...
$.extend(wishlist, {
	...
	add_remove_from_wishlist(action, args, success_action, failure_action, async = false) {
		...
		if (frappe.session.user === "Guest") {
			...
			let method = "shop.shop.doctype.wishlist.wishlist.add_to_wishlist";
			if (action === "remove") {
				method = "shop.shop.doctype.wishlist.wishlist.remove_from_wishlist";
			}
