	...
	function apply_loyalty_points() {
		...
		const callback = async (r) => {
			...
			if (doc_info.grand_total && doc_info.grand_total < loyalty_amount) {
				...
				message =
					"You can only redeem max " + redeemable_amount + " points in this order.";
				frappe.msgprint(__(message));
				return;
			}

			message =
				loyalty_points + " Loyalty Points of amount " + loyalty_amount + " is applied.";
			...
			const payment_gateway_account = await frappe.db.get_single_value(
				"Webshop Settings",
			...
			const args_str = Object.entries(args_obj)
				.map((e) => e[0] + "=" + e[1])
				.join("&");
