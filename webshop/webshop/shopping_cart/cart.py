...
from frappe.utils import cint, cstr, flt, get_fullname, get_url
...
def place_order():
    ...
    # Snapshot the quotation data into a cache token
    token = str(uuid.uuid4())

    # Reject if this quotation already has a completed SO
    existing_so = frappe.db.get_value(
        "Sales Order", {"po_no": quotation.name, "docstatus": 1}, "name"
    )
    if existing_so:
        ...
        if has_pe:
            frappe.throw(
                _(
                    "This cart has already been paid. Please refresh the page to start a new order."
                ),
    ...
    # Store in Frappe cache for 30 minutes
    frappe.cache().set_value(
        f"momo_pending_{token}", json.dumps(cart_data), expires_in_sec=1800
    )

    # Get gateway
    momo_gateway = get_default_momo_gateway()
    controller = get_payment_gateway_controller("MoMo-" + momo_gateway)
    ...
    params = urlencode(
        {
            ...
            "currency": quotation.currency,
            "gateway_name": momo_gateway,
            "title": f"Order from {quotation.customer_name}",
        }
    )
    payment_url = get_url(f"/momo_checkout?{params}")
...
def get_default_momo_gateway():
    """Return the first active MoMo Settings gateway name."""
    momo = frappe.db.get_value("MoMo Settings", {}, "gateway_name")
    if not momo:
        frappe.throw(_("No MoMo gateway configured in MoMo Settings"))
    return momo


def create_momo_payment_request(sales_order_name, gateway_name):
    """Create a draft Payment Request for a Sales Order (no auto‑push)."""
    so = frappe.get_doc("Sales Order", sales_order_name)
    gateway_full = (
        "MoMo-" + gateway_name if not gateway_name.startswith("MoMo-") else gateway_name
    )

    pga = frappe.db.get_value(
        "Payment Gateway Account",
        {"payment_gateway": gateway_full},
    ...
    pr.payment_gateway = gateway_full
    pr.payment_account = pga.payment_account if pga else ""
    pr.currency = so.currency
    pr.grand_total = so.grand_total
    pr.base_grand_total = so.base_grand_total
    pr.outstanding_amount = so.grand_total
    pr.base_outstanding_amount = so.base_grand_total
    pr.email_to = so.customer
    pr.subject = f"Payment Request for {so.name}"

    # IMPORTANT: Do NOT submit (docstatus=0) so that the automatic push is not triggered.
...
def remove_coupon_code():
    ...
    # reset discount amount if coupon code is removed
